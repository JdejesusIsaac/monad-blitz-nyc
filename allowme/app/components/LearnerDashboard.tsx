...
type LearnerCourse = {
  ...
  payout: { txHash: string | null; amountUsdc: string; explorerLink: string | null } | null;
};
  ...
  if (isConnected && address) {
    return (
      <div className="relative">
        ...
        >
          <span className="w-2 h-2 rounded-full shrink-0" style={{ background: "#4ade80", boxShadow: "0 0 5px 1px rgba(74,222,128,0.55)" }} />
        ...
        {open && (
          <div className="absolute right-0 mt-1 rounded-xl py-1 z-20" style={{ background: "#fff", border: "1px solid #e5e7eb", boxShadow: "0 8px 24px rgba(0,0,0,0.1)", minWidth: "190px" }}>
            <div className="px-4 py-2.5" style={{ borderBottom: "1px solid #f3f4f6" }}>
              ...
              <div style={{ fontSize: "0.78rem", color: "#374151", fontWeight: 500, fontFamily: "monospace", marginTop: "2px" }}>{truncate(address)}</div>
            </div>
            <button
...
export function LearnerDashboard() {
  ...
  useEffect(() => {
    if (!address) { setLearnerData(null); return; }
    fetch(`/api/dashboard/learner?wallet=${address}`)
      .then((r) => r.json())
      .then(setLearnerData)
      .catch(console.error);
  }, [address]);

  // Derive completed + available courses from API data
  const completedCourses = learnerData?.courses.filter((c) => c.completed) ?? [];
  const completedSlugs = new Set(completedCourses.map((c) => c.courseSlug));
  const availableCourses = AVAILABLE_COURSES.filter((c) => !completedSlugs.has(c.id));
  const showAvailable = availableCourses.length > 0 || !address;

  const totalEarned = learnerData ? formatMon(learnerData.totalEarnedUsdc) : "0 MON";
  const avgScore = completedCourses.length > 0
    ? Math.round(completedCourses.reduce((s, c) => s + (c.score ?? 0), 0) / completedCourses.length)
    : null;
  const onChainTxns = learnerData?.recentPayouts.filter((p) => p.txHash).length ?? 0;
    ...
    <div className="flex size-full min-h-screen" style={{ fontFamily: "'Inter', sans-serif", background: "#f8f9fb" }}>
      ...
      <aside className="flex flex-col w-52 shrink-0 min-h-screen" style={{ background: "#fff", borderRight: "1px solid #e5e7eb" }}>
        ...
        <div className="px-4 pb-5">
          <div className="rounded-xl p-3" style={{ background: "#f3f4f6" }}>
            <div style={{ fontSize: "0.7rem", fontWeight: 600, color: "#9ca3af", letterSpacing: "0.07em", marginBottom: "4px" }}>LEARNER</div>
            {isConnected && address ? (
              <>
                <div style={{ fontSize: "0.78rem", fontWeight: 600, color: "#374151", fontFamily: "monospace" }}>{truncate(address)}</div>
                <div style={{ fontSize: "0.72rem", color: "#16a34a", marginTop: "2px", fontWeight: 500 }}>Wallet connected ✓</div>
              </>
            ) : (
      ...
      <main className="flex-1 flex flex-col overflow-auto">
        ...
        <header className="flex items-center justify-between px-8 py-4 bg-white" style={{ borderBottom: "1px solid #e5e7eb" }}>
          <div>
            <h1 style={{ fontWeight: 800, fontSize: "1.15rem", color: "#111827", letterSpacing: "-0.02em" }}>Learner Dashboard</h1>
            <p style={{ fontSize: "0.78rem", color: "#9ca3af", fontWeight: 400, marginTop: "1px" }}>Complete courses. Get verified. Receive MON.</p>
          </div>
          <ConnectWalletButton
        ...
        <div className="flex-1 px-8 py-7 flex flex-col gap-7">
          ...
          {/* Payout address banner */}
          {isConnected && address && (
            <div className="flex items-center gap-2 rounded-xl px-4 py-3" style={{ background: "#f0fdf4", border: "1px solid #bbf7d0" }}>
          ...
          <div className="grid grid-cols-4 gap-4">
            <div className="rounded-2xl p-5 flex flex-col justify-between" style={{ background: "linear-gradient(135deg, #4f46e5 0%, #7c3aed 100%)", color: "#fff" }}>
              <div style={{ fontSize: "0.68rem", fontWeight: 700, letterSpacing: "0.09em", opacity: 0.7 }}>TOTAL EARNED</div>
              <div>
                <div style={{ fontWeight: 800, fontSize: "2rem", letterSpacing: "-0.03em", lineHeight: 1 }}>
                  {address ? (learnerData ? totalEarned.split(" ")[0] : "…") : "—"}
                </div>
                <div style={{ fontSize: "0.8rem", fontWeight: 600, opacity: 0.75, marginTop: "2px" }}>MON</div>
              </div>
              <div style={{ fontSize: "0.72rem", opacity: 0.6, fontWeight: 500 }}>
                {address ? `${completedCourses.length} course${completedCourses.length !== 1 ? "s" : ""} completed` : "Connect wallet to view"}
              </div>
            </div>
            ...
            {[
              {
                ...
                value: address ? (learnerData ? String(completedCourses.length) : "…") : "—",
                sub: `of ${AVAILABLE_COURSES.length + completedCourses.length} available`,
              },
              {
                label: "Avg Score",
                value: address ? (learnerData ? (avgScore !== null ? `${avgScore}%` : "—") : "…") : "—",
                sub: "across all quizzes",
              },
              {
                label: "On-chain Txns",
                value: address ? (learnerData ? String(onChainTxns) : "…") : "—",
                sub: "confirmed on Monad",
              },
            ].map((s) => (
              <div key={s.label} className="rounded-2xl bg-white p-5" style={{ border: "1px solid #e5e7eb" }}>
                <div style={{ fontSize: "0.68rem", fontWeight: 600, color: "#9ca3af", letterSpacing: "0.07em", marginBottom: "10px", textTransform: "uppercase" }}>{s.label}</div>
                <div style={{ fontWeight: 800, fontSize: "1.45rem", color: "#111827", letterSpacing: "-0.02em", lineHeight: 1 }}>{s.value}</div>
          ...
          <div className="flex gap-6 items-start">
            ...
            {showAvailable && (
              <div className="flex flex-col gap-3 shrink-0" style={{ width: "320px" }}>
                ...
                {availableCourses.map((course) => (
                  <div key={course.id} className="rounded-2xl bg-white p-5 flex flex-col gap-4" style={{ border: "1px solid #e5e7eb" }}>
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <div style={{ fontWeight: 700, fontSize: "0.92rem", color: "#111827" }}>{course.title}</div>
                        <div style={{ fontSize: "0.74rem", color: "#9ca3af", marginTop: "3px", lineHeight: 1.55 }}>{course.description}</div>
                      </div>
                      <span className="rounded-full px-2.5 py-0.5 shrink-0" style={{ background: "#f0fdf4", color: "#15803d", fontSize: "0.68rem", fontWeight: 700, border: "1px solid #bbf7d0" }}>
                        {course.difficulty}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center justify-between pt-3" style={{ borderTop: "1px solid #f3f4f6" }}>
                      <span className="rounded-full px-3 py-1" style={{ background: "#f0fdf4", color: "#16a34a", fontSize: "0.78rem", fontWeight: 700, border: "1px solid #bbf7d0" }}>
                      ...
                      <Link
                        ...
                        style={{ background: "linear-gradient(135deg, #4f46e5, #7c3aed)", color: "#fff", fontWeight: 600, fontSize: "0.8rem", textDecoration: "none", boxShadow: "0 2px 10px rgba(79,70,229,0.3)" }}
                      >
                        Start Quiz
            ...
            {completedCourses.length > 0 && (
              <div className="flex flex-col gap-3 flex-1">
                ...
                {completedCourses.map((course) => (
                  <div key={course.programId} className="rounded-2xl bg-white p-5 flex flex-col gap-3" style={{ border: "1px solid #e5e7eb" }}>
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div style={{ fontWeight: 700, fontSize: "0.88rem", color: "#111827" }}>AI Ethics 101</div>
                          <div style={{ fontSize: "0.73rem", color: "#9ca3af", marginTop: "1px" }}>5 sections · 17 questions</div>
                        </div>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        {course.score !== null && (
                          <span className="rounded-full px-2.5 py-0.5" style={{ background: "#dcfce7", color: "#15803d", fontSize: "0.72rem", fontWeight: 700 }}>
                            {course.score}% score
                          </span>
                        )}
                        <span className="rounded-full px-2.5 py-0.5" style={{ background: "#f0fdf4", color: "#16a34a", fontSize: "0.72rem", fontWeight: 600, border: "1px solid #bbf7d0" }}>
                          {formatMon(course.rewardAmountUsdc)} {course.payout ? "paid" : "pending"}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between pt-2" style={{ borderTop: "1px solid #f3f4f6" }}>
                      <span style={{ fontSize: "0.73rem", color: "#9ca3af" }}>
                        {course.verifiedAt ? `Completed ${new Date(course.verifiedAt).toLocaleDateString()}` : "Completed"}
                      </span>
                      {course.payout?.txHash ? (
                        <a
                          href={course.payout.explorerLink ?? `https://testnet.monadvision.com/tx/${course.payout.txHash}`}
                          target="_blank"
                        ...
                        <Link
                          href="/learner/quiz"
                          style={{ fontSize: "0.75rem", color: "#4f46e5", fontWeight: 500, textDecoration: "none" }}
                        >
                          Claim reward →
                        </Link>
                      )}
            ...
            {/* Empty state when wallet connected but no completions */}
            {address && learnerData && completedCourses.length === 0 && availableCourses.length === 0 && (
              <div className="flex-1 rounded-2xl bg-white flex flex-col items-center justify-center py-16 gap-2" style={{ border: "1px solid #e5e7eb" }}>
                <span style={{ fontSize: "0.85rem", color: "#9ca3af" }}>No completed courses yet — take a quiz!</span>
              </div>
            )}
          </div>
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                style={{ background: "linear-gradient(135deg, #4f46e5, #7c3aed)", color: "#fff", fontWeight: 600, fontSize: "0.8rem", border: "none", cursor: "pointer", boxShadow: "0 2px 10px rgba(79,70,229,0.3)" }}
              >
                <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
            ...
            <div className="grid grid-cols-4 gap-4">
              {[
                ...
                <div key={module.week} className="rounded-2xl p-5 flex flex-col gap-3 relative overflow-hidden" style={{ background: "#fff", border: "1px solid #e5e7eb", opacity: 0.85 }}>
                  ...
                  <div>
                    <div style={{ fontSize: "0.68rem", fontWeight: 600, color: "#9ca3af", letterSpacing: "0.07em", marginBottom: "3px" }}>{module.week.toUpperCase()}</div>
          ...
          <div className="rounded-2xl bg-white overflow-hidden" style={{ border: "1px solid #e5e7eb" }}>
            ...
            {(!address || (learnerData && learnerData.recentPayouts.length === 0)) ? (
              <div className="flex items-center justify-center py-10">
                <span style={{ fontSize: "0.85rem", color: "#9ca3af" }}>
                  {address ? "No payouts yet — complete a quiz to earn MON" : "Connect wallet to view your payouts"}
                </span>
              </div>
            ) : learnerData ? (
              <table className="w-full">
                <thead>
                  <tr style={{ background: "#fafafa" }}>
                    {["Course", "Amount", "Tx Hash", "Date"].map((col) => (
                      <th key={col} className="text-left px-6 py-3" style={{ fontSize: "0.68rem", fontWeight: 600, color: "#9ca3af", letterSpacing: "0.07em", textTransform: "uppercase" }}>
                        {col}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {learnerData.recentPayouts.map((row) => (
                    <tr key={row.id} className="hover:bg-gray-50 transition-colors" style={{ borderTop: "1px solid #f3f4f6" }}>
                      <td className="px-6 py-4" style={{ fontSize: "0.83rem", color: "#374151", fontWeight: 500 }}>AI Ethics 101</td>
